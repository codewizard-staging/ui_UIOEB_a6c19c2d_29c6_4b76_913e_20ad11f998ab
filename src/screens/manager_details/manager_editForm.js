...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], managerId, manager, numfields, mapItem;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            const mapPropKey = manager.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'manager');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
